...
class UserPaymentsController extends Controller
{
    ...
    public function store(PaymentRequest $request, $user_id) {
        ...
        return redirect()->route('users.payments', ['user_id' => $user_id])->with(['message' => 'Payment Added Successfully']);
    }

    public function destroy($user_id, $payment_id) {
        Payment::destroy($payment_id);
        return redirect()->route('users.payments', ['user_id'=>$user_id])->with(['message'=>'Payment Deleted Successfully']);
    }
}